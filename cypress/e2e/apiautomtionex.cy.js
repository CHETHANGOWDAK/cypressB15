...
describe('Automation - API Automation - GET. PUT, POST and Delete Methods', function () {
  ...
  it.only("Get users", () => {
    cy.request("GET", "https://reqres.in/api/users?page=2").then((response) => {
      ...
      expect(response.body.data[4].email).equal("george.edwards@reqres.in",);
      //for url verification
      expect(response.body.support.url).equal("https://reqres.in/#support-heading",);
      //some data available are not verification
      expect(response.body).to.have.property('total_pages');
      //for verifying total pages 
      expect(response.body).to.have.property('total_pages', 2);
      //for verification some name
      expect(response.body.data[0].first_name).equal("Michael");
      //for url verification
      expect(response.body.support.url).equal("https://reqres.in/#support-heading");
      // for page property verification
      expect(response.body).to.have.property('total_pages');
      expect(response.body).to.have.property('total_pages', 2);

      // (response.body).to.have.property("page", 2)
  ...
  it.only('Cypress Test Case - Understanding GET Method', function () {
    ...
    cy.request({
      ...
      headers: {
        ...
        "Cookie": "orangehrm=dc8e93f392d45d1ed7b235576db13221",
